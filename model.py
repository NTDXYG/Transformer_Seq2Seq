...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, beam_size=None, max_length=None, sos_id=None, eos_id=None):
        ...
        self.position_encoding = nn.Embedding(config.max_position_embeddings, config.hidden_size)
    ...
    def forward(self, source_ids=None, source_mask=None, target_ids=None, target_mask=None):
        ...
        position_embedding = torch.arange(0, word_embedding.shape[1]).unsqueeze(0).repeat(word_embedding.shape[0], 1).to(word_embedding.device)
        position_embedding = self.position_encoding(position_embedding)
        ...
        if target_ids is not None:
            ...
            for i in range(source_ids.shape[0]):
                ...
                for _ in range(self.max_length):
                    ...
                    position_embedding = torch.arange(0, word_embedding.shape[1]).unsqueeze(0).repeat(
                        word_embedding.shape[0], 1).to(word_embedding.device)
                    position_embedding = self.position_encoding(position_embedding)
                    tgt_embeddings = word_embedding + position_embedding
                    tgt_embeddings = tgt_embeddings.permute([1, 0, 2]).contiguous()
...
class Transformer_Seq2Seq():
    def __init__(self, codebert_path, encoder_layers, decoder_layers, beam_size, max_source_length, max_target_length, load_model_path):
        ...
        encoder = nn.TransformerEncoder(encoder_layer, num_layers=encoder_layers)
